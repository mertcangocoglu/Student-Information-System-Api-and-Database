...
namespace WebApiProje.Controllers
{
    ...
    public class UnvanController : ControllerBase
    {
        ...
        public UnvanController(IDbConnection dbConnection, IConfiguration configuration)
        {
        ...
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(IDataResult<IEnumerable<Unvan>>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(Core.Toolkit.Results.IResult))]
        [HttpPost("/unvan/search")]
        public async Task<IActionResult> Search(UnvanRequest request)
        {
            var records = await new DapperRepository(_dbConnection).GetMultipleAsync(
                 GetUnvanSqlQueryBuilderExtensions
                .GetUnvanSqlQuery(request));

            var datas = await records.ReadAsync<Unvan>();
            var count = await records.ReadFirstOrDefaultAsync<int>();

            var result = new SuccessDataResult<IEnumerable<Unvan>>(datas, count);
            if (result.IsSuccess)
            {
                return Ok(result);
            }
