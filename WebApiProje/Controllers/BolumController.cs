...
namespace WebApiProje.Controllers
{
    ...
    public class BolumController : ControllerBase
    {
        ...
        public BolumController(IDbConnection dbConnection, IConfiguration configuration)
        {
        ...
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(IDataResult<IEnumerable<Bolum>>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(Core.Toolkit.Results.IResult))]
        [HttpGet("/bolum/search")]
        public async Task<IActionResult> Search([FromQuery] BolumRequest request)
        {
            var records = await new DapperRepository(_dbConnection).GetMultipleAsync(
                 GetBolumSqlQueryBuilderExtensions
                .GetBolumSqlQuery(request));

            var datas = await records.ReadAsync<Bolum>();
            var count = await records.ReadFirstOrDefaultAsync<int>();

            var result = new SuccessDataResult<IEnumerable<Bolum>>(datas, count);
            if (result.IsSuccess)
            {
                return Ok(result);
            }
